...
- **Text anchored at a plot edge is offset 2px from it.** Applies to `ds.add_rule()` labels, `ds.add_text()` position presets, and the test and correlation labels built on them. `ds.add_rule()` previously offset its labels on closed plots only.
