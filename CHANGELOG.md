...
- **`ds_div_3`: a diverging companion to `ds_cat_3`.** Purple for low values, green for high, meeting at a neutral near-white - built from the same `cat3_purples` and `cat3_greens` ramps, so it sits in the `ds_cat_3` register the way `ds_div_1` does for `ds_cat_1`. Opt in with `ds.theme(divergingPalette="ds_div_3")`. Green and purple were chosen over the palette's teal or blue: those sit on the same side of the blue-yellow axis as purple, so under simulated deuteranopia the two arms collapse into one hue and the sign of a value becomes unreadable. Purple and green stay 166 degrees apart under the same simulation, and both arms remain lightness-monotonic.
...
- **A p-value bracket now clears every group it spans, not just the two it compares.** A bracket from the first to the third category passes over the second, so a taller group in the middle sat above the bracket - on a bar chart the bracket crossed straight through that bar. The anchor is now the maximum over every category the bracket spans. Applies to grouped (`xOffsetCol`) brackets over their sub-bars too.
