...
- **`reverse=` brackets hang below their data again.** They anchored on the groups' maximum and then hung downward from it, which dropped the bracket and its label onto the points. A reversed bracket now anchors on the spanned minimum and stacks downward, with the two directions resolved independently so brackets above and below one chart never push each other around.
