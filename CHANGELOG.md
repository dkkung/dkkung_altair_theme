...
- **`add_multilabel(rowAngle=)`** rotates a condition-table row's text - `rowAngle={"dose": -90}` stands a row of numeric values on end so long values fit under narrow categories. Takes a single angle for every row, a dict keyed by row label, or a list in row order. A row's own angle may be a list too, one per x-axis category, to rotate only some cells - `rowAngle={"dose": [0, 0, -90, -90, -90]}` stands the doses on end while leaving the untreated controls' `-` placeholders upright. Text rotates about its own center, so it stays centered on its category, and a rotated row grows to fit its tallest rotated cell automatically. `rowHeight` now accepts a dict or list too, to pin heights per row.
